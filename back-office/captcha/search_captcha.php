...
try {
    if (!empty($search)) {
        ...
        $stmt = $bdd->query("SELECT id_captcha, question,answer,status,id_auteur,email FROM captcha join utilisateurs where id_utilisateurs=captcha.id_auteur ORDER BY id_captcha");
    }
