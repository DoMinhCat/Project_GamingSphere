...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (strlen($titre) > 150 || strlen($contenu) > 1000) {
        header('Location:' . forum_discussion_back . '?error=length');
        exit;
    } elseif (empty($titre) || empty($contenu)) {
        header('Location:' . forum_discussion_back . '?error=missing_fields');
        exit;
    } else {
        try {
            $stmt = $bdd->prepare("INSERT INTO forum_sujets (titre, contenu, date_msg, categories, parent_id, auteur) VALUES (?, ?, NOW(), 'Discussions', NULL, ?);");
            $stmt->execute([$titre, $contenu, $auteur]);

            header('Location:' . forum_discussion_back . '?message=added');
            exit;
        } catch (PDOException $e) {
            $_SESSION['error'] = htmlspecialchars($e->getMessage());
            header('Location:' . forum_discussion_back . '?error=bdd');
            exit();
        }
    }
}
if (!empty($_GET['delete_id'])) {
    ...
    try {
        ...
        header('Location:' . forum_discussion_back . '?message=deleted');
        exit();
    } catch (PDOException $e) {
        $_SESSION['error'] = htmlspecialchars($e->getMessage());
        header('Location:' . forum_discussion_back . '?error=bdd');
...
try {
    $query = $bdd->prepare("SELECT id_sujet,titre, date_creation, auteur from forum_sujets where categories='Discussions' order by date_creation desc;");
    $query->execute();
...
<body class="pb-4">
    ...
    <main class="container mb-5">
        ...
        <h3 class="text-center mt-5 mb-4">Liste des sujets Discussion</h3>
        <?php
        ...
        if (count($annonces) > 0) {
            ...
            foreach ($annonces as $annonce) {
                echo '<tr>
                        ...
                        <td>
                            <a href=' . discussion_edit_back . '?id=' . $annonce['id_sujet'] . ' class="btn btn-sm btn-warning my-1 me-1">Modifier</a>
                            ...
                            <div class="modal-dialog">
                              <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                  ...
                                  <a href="' . forum_discussion_back . '?delete_id=' . $annonce['id_sujet'] . '" class="btn btn-danger">Supprimer</a>
                                </div>
                              </div>
                            </div>
                          </div>';
    ...
    <script>
        ...
        document.getElementById('search').addEventListener('input', fetchDiscussions);
