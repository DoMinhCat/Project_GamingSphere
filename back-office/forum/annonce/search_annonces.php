...
try {
    if (!empty($search)) {
        $stmt = $bdd->prepare("SELECT id_sujet,titre, date_creation, auteur from forum_sujets where categories='Annonces' and (titre LIKE :search or auteur LIKE :search) order by date_creation desc;");
        $stmt->execute(['search' => '%' . $search . '%']);
    } else {
        $stmt = $bdd->query("SELECT id_sujet,titre, date_creation, auteur from forum_sujets where categories='Annonces' order by date_creation desc;");
    }
