...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['annonceId'])) {
    ...
    if (empty($titre) || empty($contenu) || empty($category)) {
        header('Location:' . support_edit_back . '?message=' . urlencode('Veuillez remplir tous les champs !'));
        exit();
    }
    ...
    try {
        ...
        header('Location:' . forum_support_back . '?message=updated');
        exit();
    ...
    header('location:' . forum_support_back . '?error=missing_id');
    exit();
}
