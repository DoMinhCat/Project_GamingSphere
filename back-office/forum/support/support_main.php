...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (strlen($titre) > 150 || strlen($contenu) > 1000) {
        header('Location:' . forum_support_back . '?error=length');
        exit;
    } elseif (empty($titre) || empty($contenu)) {
        header('Location:' . forum_support_back . '?error=missing_fields');
        exit;
    } else {
        try {
            $stmt = $bdd->prepare("INSERT INTO forum_sujets (titre, contenu, date_msg, categories, parent_id, auteur) VALUES (?, ?, NOW(), 'Support', NULL, ?);");
            $stmt->execute([$titre, $contenu, $auteur]);

            header('Location:' . forum_support_back . '?message=added');
            exit;
...
if (!empty($_GET['delete_id'])) {
    ...
    try {
        ...
        header('Location:' . forum_support_back . '?message=deleted');
        exit();
    } catch (PDOException $e) {
        $_SESSION['error'] = htmlspecialchars($e->getMessage());
        header('Location:' . forum_support_back . '?error=bdd');
...
try {
    $query = $bdd->prepare("SELECT id_sujet,titre, date_creation, auteur from forum_sujets where categories='Support' order by date_creation desc;");
    $query->execute();
...
<body class="pb-4">
    ...
    <main class="container mb-5">
        ...
        <h1 class="my-5 text-center">Gestion du forum Support</h1>
        <form method="post">
            <div class="mb-3">
        ...
        <h3 class="text-center mt-5 mb-4">Liste des sujets Support</h3>
        <?php
    ...
    <script>
        ...
        document.getElementById('search').addEventListener('input', fetchSupport);
