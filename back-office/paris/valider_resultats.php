...
if (isset($_POST['id_tournoi'], $_POST['id_gagnant'])) {
    ...
    // Détermine si le tournoi est en solo ou en équipe
    $stmt = $bdd->prepare("SELECT type FROM tournoi WHERE id_tournoi = ?");
    $stmt->execute([$id_tournoi]);
    $type = $stmt->fetchColumn();
    $is_team = ($type === 'equipe');

    // Vérifie le nombre de participants au tournoi
    ...
    foreach ($paris as $pari) {
        ...
        // Si un seul participant, il ne peut pas perdre
        ...
        // Créditer le gain si pari gagnant
        if ($gagne && $gain > 0) {
            $stmt3 = $bdd->prepare("
    ...
    // Enregistre le résultat du tournoi
    if ($is_team) {
